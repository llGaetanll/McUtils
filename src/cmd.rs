use crate::util::{BlockPoint, Point2D, Point3D, Repr2D};
...
pub fn fill(p1: &BlockPoint, p2: &BlockPoint, block: &Block, params: CmdParams) -> Command {
    ...
    format!(
        "fill {} {} {} {} {} {} {}{}",
        p1.x, p1.y, p1.z, p2.x, p2.y, p2.z, block, params
    )
}
 ...
 **/
pub fn walls_3d(c: &Point2D<i32>, sidelength: i32, block: &Block) -> Vec<Command> {
    let mut cmds = Vec::new();
    ...
    for i in 0..verts.len() {
        ...
        cmds.push(fill(
            &verts[i].to_3d(0, Repr2D::XZ),
            &verts[(i + 1) % 4].to_3d(255, Repr2D::XZ),
            block,
            None,
        ));
    }
...
pub fn chain(cmds: Vec<Command>) -> Command {
    ...
    format!(
        "summon falling_block ~ ~1 ~ {{Time:1,BlockState:{{Name:activator_rail}},Passengers:[{}]}}",
        passengers
    )
}
